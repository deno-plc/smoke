import { type HttpRequestInfo, Network } from "@sinclair/smoke";
...
async function selectRequestProperty<
  T extends (info: HttpRequestInfo) => unknown,
...
Test.describe("Http:Request:RequestInfo", () => {
  ...
  Test.it("Should have local hostname", async () => {
    const value = await selectRequestProperty((request) =>
