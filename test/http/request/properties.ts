...
async function resolveRequestObject<T>(
  path: string,
  method: string,
  body: string | null,
  ...
  await Http.fetch(`http://localhost:5000${path}`, { method, body, headers })
    .then((r) => r.text());
...
Test.describe("Http:Request:Properties", () => {
  ...
  Test.before(async () => {
    cloneRequest = await resolveRequestObject(
      "/test",
      "POST",
      "hello",
      (request) => request.clone(),
    );
    getRequest = await resolveRequestObject(
      "/test",
      "GET",
      null,
      (request) => request,
    );
    postRequest = await resolveRequestObject(
      "/test",
      "POST",
      "hello",
      (request) => request,
    );
    pathRequest1 = await resolveRequestObject(
      "/",
      "GET",
      null,
      (request) => request.clone(),
    );
    pathRequest2 = await resolveRequestObject(
      "/a/b/c?x=10",
      "GET",
      null,
      (request) => request.clone(),
    );
    pathRequest3 = await resolveRequestObject(
      "/a/b/c?x=10#hash",
      "GET",
      null,
      (request) => request.clone(),
  ...
  Test.it("Should have redirect", async () => {
    Assert.isTrue(
      getRequest.redirect === undefined ||
        typeof getRequest.redirect === "string",
    );
  });
  // ----------------------------------------------------------------
  // referrer
  ...
  Test.it("Should have referrer", async () => {
    Assert.isTrue(
      getRequest.referrer === undefined ||
        typeof getRequest.referrer === "string",
    );
  });
  // ----------------------------------------------------------------
  // referrerPolicy
  ...
  Test.it("Should have referrer", async () => {
    Assert.isTrue(
      getRequest.referrerPolicy === undefined ||
        typeof getRequest.referrerPolicy === "string",
    );
  });
  // ----------------------------------------------------------------
  // signal
