...
async function echo(inputs: any[]): Promise<any[]> {
  const listener = Http.listen(
    { port: 5000 },
    async (request) => new Response(request.body),
  );
  const [endpoint, method, headers] = [`http://localhost:5000/`, "post", {
    "Content-Type": "application/json",
  }];
  const buffers = await Promise.all(
    inputs.map((input) => JSON.stringify(input)).map((body) =>
      Http.fetch(endpoint, { headers, method, body }).then((res) => res.json())
    ),
  );
