import { Buffer, Proxy } from "@sinclair/smoke";
...
Test.describe("Proxy", () => {
  Test.before(async () => {
    ...
    await Proxy.listen(
      { path: "/status" },
      () => new Response("status", { status: 400, statusText: "400" }),
    );
    await Proxy.listen(
      { path: "/echo" },
      (request) => new Response(request.body),
    );
  });
  // ----------------------------------------------------------------
  // Root
  ...
  Test.it("should fetch echo (string)", async () => {
    const result = await fetch("/echo", { method: "POST", body: "hello" }).then(
      (res) => res.text(),
    );
    Assert.isEqual(result, "hello");
  });
  Test.it("should fetch echo (blob)", async () => {
    const result = await fetch("/echo", {
      method: "POST",
      body: new Blob(["hello"]),
    }).then((res) => res.text());
    Assert.isEqual(result, "hello");
  ...
  async function readableEcho() {
    ...
    const result = await fetch(
      "/echo",
      { method: "POST", body: readable, duplex: "half" } as any,
