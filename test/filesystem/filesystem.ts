import { Buffer, type Crypto, FileSystem, Path } from "@sinclair/smoke";
import { Assert, Hash, Test } from "../test/index.ts";
...
Test.describe("FileSystem", () => {
  ...
  Test.it("Should throw on invalid read range 2", async () => {
    ...
    await Assert.shouldThrowAsync(async () =>
      await fs.readable(path, 201, 200)
    );
  });
  // ----------------------------------------------------------------
  // Readir
  ...
  Test.it(
    "Should throw when creating directory under file path 1",
    async () => {
      const path = "/folder";
  ...
  Test.it(
    "Should throw when creating directory under file path 2",
    async () => {
      const path = "/folder";
      await fs.write(`${path}`, Buffer.alloc(16));
      await Assert.shouldThrowAsync(async () =>
        await fs.write(`${path}/directory/file`, Buffer.alloc(10))
      );
    },
  );
  // ----------------------------------------------------------------
  ...
  Test.it("Should throw invalid path 1", async () => {
    await Assert.shouldThrowAsync(async () =>
      await fs.write("/a/../test", Buffer.alloc(0))
    );
  });
  ...
  // ----------------------------------------------------------------
  Test.it(
    "Should throw if creating a file at folder location (root)",
    async () => {
      await fs.mkdir("/foo");
  ...
  Test.it(
    "Should throw if creating a folder at file location (root)",
    async () => {
  ...
  Test.it(
    "Should throw if creating a file at folder location (deep)",
    async () => {
  ...
  Test.it(
    "Should throw if creating a folder at file location (deep)",
    async () => {
